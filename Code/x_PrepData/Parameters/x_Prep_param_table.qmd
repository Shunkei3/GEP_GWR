...
```{r}
library(here)
i_am("Scr/Code/x_PrepData/Parameters/x_Prep_param_table.qmd")
...
flow_acc_tbl <- 
  readRDS(file.path(int_dir, "FlowAcc/flow_acc_tbl_global.rds")) %>%
  setnames("region_abbr", "region")


# === beta_i and gamma parameters === #
beta_gamma_tbl <-
  list.files(
    file.path(int_dir, "Params"),
...
param_tbl <-
  readRDS(here(int_dir, "SWY_inputs/0_param_tbl.rds"))
```