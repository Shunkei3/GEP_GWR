...
fn_comp_prep_inputs <- function(aoi_sf_var, save_dir_return = FALSE, custom_aoi_provided = FALSE, custom_region_abbr_var = NULL, custom_save_dir = NULL){
  #  aoi_sf_var <- tmp_aoi_i
  ...
  if(custom_aoi_provided){
    ...
    # aoi_sf_var <- st_read(here(tmp_save_dir, "aoi_guara_test.gpkg"))
    # custom_region_abbr_var <- "af"
    # custom_save_dir <- tmp_save_dir

    region_abbr <- custom_region_abbr_var
    save_dir <- custom_save_dir
  
  } else {
    region_abbr <- aoi_sf_var$continent # this will be used through out the functions
    #  aoi_hybas_id <- aoi_sf_var$HYBAS_ID
    ...
    save_dir <-
      fn_gen_save_dir(
        where = swy_dt_dir, 
        new_relative_dir_path = file.path(region_abbr, aoi_sf_var$HYBAS_ID),
        return_path = TRUE
      )
  }
...
```{r}
fn_get_dem_aoi <- function(aoi_sf_var, save_dir_var){
  # aoi_sf_var <- tmp_aoi_i
  # save_dir_var <- save_dir

  dem_aoi <- 
    rast(file.path(raw_dir, "DEM", paste0("hyd_", region_abbr, "_dem_15s.tif"))) %>%
    crop(
      ., vect(st_transform(aoi_sf_var, crs = crs(., proj = TRUE))),
      mask = TRUE
    ) %>%
...
```{r}
get_beta_twi <- function(aoi_sf_var, save_dir_var){
  ...
  flow_acc <-
    rast(file.path(raw_dir, "FlowAccumulation", paste0("hyd_", region_abbr, "_acc_15s.tif"))) %>%
    crop(
        ., vect(st_transform(aoi_sf_var, crs = crs(., proj = TRUE)))
        # mask = TRUE
      ) %>%
