...
# /*===========================================*/
# Crop -> Mask
# crop first to speed things up (remove unnecessary data outside bounding box)
# mask next to accurately blank out everying outside the polygon boundary.
...
fn_get_lulc_aoi <- function(aoi) {
  ...
  lulc_aoi[is.na(lulc_aoi)] <- 999 # you should not do this
...
# See data section on evapotranspiration: it says "If you are running a model that requires both precipitation and evapotranspiration as inputs, make sure that the ET data is based on the same precipitation data that is being used as a model input."
...
fn_get_cn_tbls <- function(aoi, lulc_aoi, soil_aoi){
  ...
  lulc_resampled[is.na(lulc_resampled)] <- 999 # I think you should not do this.
