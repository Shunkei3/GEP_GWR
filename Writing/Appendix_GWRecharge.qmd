...
<!-- Overview -->
I will conduct the analysis on a watershed scale. The watershed is a common scale for hydrological analysis because it is a natural hydrological unit that collects precipitation and drains it into a common outlet, such as a river or lake. Not all those watersheds in the world are relevant for the current analysis, as only those that intersect regions with significant groundwater dependence, where groundwater serves as a essential source of water for human use, are pertinent for evaluating the value of groundwater recharge service. To identify such watersheds, I use two datasets: 



To this end, I use two datasets: one is the major groundwater basin boundary dataset obtained from @Niazi.etal2024, and the other is the watershed boundary data from HydroBASINS v1 (Level 6) [@Lehner.Grill2013]. Specifically, I identified the watersheds that overlap with major groundwater basins. In total, 7,722 watersheds are selected for the analysis. 

**insert a figure here**

Then, for each watershed, I estimate the amount of natural groundwater recharge provided from natural ecosystems using the InVEST Seasonal Water Yield (SWY) model.
