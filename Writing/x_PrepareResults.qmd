...
ctry_gep_sf <-
  merge(
    ctry_bd, ground_water_gep,
    by.x = "iso3c", by.y = "iso3_r250_label",
    all.x = TRUE
  ) %>%
  mutate(
    ground_water_gep_thou = ground_water_gep / 1e3
  )
...
fig_map_gep <- 
  ...
  geom_sf(
    data = ctry_gep_sf,
    aes(fill = ground_water_gep_thou), color = "black",
    linewidth = 0.2
  ) +
  geom_sf(
...
for (r in ls_regions){
  ...
  # thre <- quantile(values(tmp_l), probs = 0.99, na.rm = TRUE)

  # ls_l_lyrs_r[[r]] <- tmp_l
  # ls_l_lyrs_r[[r]] <- clamp(tmp_l, lower = 0, upper = thre)
  ls_l_lyrs_r[[r]] <- clamp(tmp_l, lower = 0, upper = 1000)
}

sammry_stat_by_r <- 
  lapply(ls_regions, function(x) {

    message("Calculating summary stats for region: ", x)

    summary(values(ls_l_lyrs_r[[x]]), na.rm = TRUE)
  })
...
```{r}
tbl_gep <- 
  ground_water_gep[, ground_water_gep := ifelse(is.na(ground_water_gep), 0, ground_water_gep)] %>%
  .[,.(
    Country = iso3_r250_name,
    `ISO3 ID` = iso3_r250_id,
    `ISO3 Code` = iso3_r250_label,
    `GEP (USD)` = ground_water_gep
  )] %>%
  flextable(.) %>%
  autofit() %>%
  add_footer_lines(values = "Note: The numbers in the column 'GEP (USD)' represent estimated Groundwater Ecosystem Product (GEP) values of natural groundwater recharge by country in 2019, expressed in 2019 USD. GEP is calculated as the monetary value of natural groundwater recharge, combined with the unit cost of groundwater extraction. Groundwater withdrawal data are from the FAO AQUASTAT Core Database (FAO, 2019). Country-level electricity prices in 2019 are used to approximate pumping energy costs. Estimates reflect direct-use value only. GEP values of zero indicate missing values of GEP due to the lack of data on groundwater withdrawal and/or electricity prices.")
  
# save_as_docx(
#   tbl_gep,
#   path = file.path(here("Scr/Writing/z_tables/gwp_gep_by_country.docx"))
# )

sum(ground_water_gep$ground_water_gep)
